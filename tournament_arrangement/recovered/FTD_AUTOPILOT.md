...
“AP”是一个明确选择后才运行的单轮自动化模式。默认的手动 JSON、FTD Console 登分、棋谱导入和 `score-scan` 流程全部保留。自动化只处理启动时选中的本地轮次/阶段；配对导入、半程比分和最终比分三个 PNG 均完成下载确认后停止，不会创建、发布、结束、删除或推进 FTD 轮次。
...
- `downloads`：只启动并分别跟踪本会话的配对、半程比分、最终比分三个 PNG 下载 ID，使用 Chrome `uniquify` 避免覆盖同名文件。
...
4. 如已知本轮开始时间，填写并“应用”；如果启动时仍为空或无效，协调器会把点击“AP”的瞬间设为开始时间。已经设定的有效时间绝不会被启动动作覆盖；
...
协调器直接从 Chrome FTD bridge 导入配对，不读取或搜索 Downloads。配对 PNG 必须直接复用“复制 FTD 导出代码”加载的 `ftd-pairing-png-renderer.js`；比分 PNG 必须直接复用“复制 FTD 登分代码”加载的 `ftd-score-png-renderer.js`。手动代码与 AP 不得各自维护 Canvas 绘制副本。它复用现有 OQ 直连推理：精确账号对、唯一时间窗对局、棋局回放、空格归胜方及超时/认输/断线结果。它从不调用微信、`score-scan`、图片识别、OCR 或 PaddleOCR。
...
有效对局桌排除 BYE。已完成并由 FTD 回读确认比分的桌数首次达到 `ceil(有效桌数 / 2)` 时，系统重新读取整轮 FTD，并用共享比分生成器输出完整配对表：已有 FTD 比分按“复制 FTD 登分代码”的规则显示，真正未完成的桌保留空白。所有桌完成后再次读取整轮 FTD；只有最终回读仍与所有比分和棋谱回执一致时，才以同一生成器绘制最终比分图。决赛阶段会把 `F` 和 `3/4` 合成同一张图。Chrome 必须分别报告配对图、半程比分图、最终比分图的下载 ID 为 `complete`，会话才进入 `done`。
...
服务器重启后只信任已经保存的精确回执。中断中的比分/棋谱命令会先回读 FTD，再决定已完成、仍为空或冲突；不会盲目重复发送。任一里程碑下载失败或不确定都会阻止继续，并且同一会话不会重复发出该里程碑的下载请求。
