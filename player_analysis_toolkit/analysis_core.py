...
def node_losses(game: dict[str, Any]) -> list[float]:
    return [float(node["lossClipped"]) for node in game["nodes"] if node.get("lossClipped") is not None]
...
def loss_game_summary(game: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "lossAtLeast4Rate": rounded(sum(value >= 4 for value in losses) / len(losses)) if losses else None,
    }
