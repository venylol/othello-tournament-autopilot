...
export const Players = ({setPressed, pressed, count = 0}) => {
    ...
    return (
        ...
        <div className="game-footer-container"> 
            <div className = {`game-footer ${opened? 'active' : ''}`} title = 'players' onClick= {clickHandler}>
                <PlayersSVG opened = {opened}/>
                <span className='viewers-counter'>{count}</span>
                <label className = {`game-footer-label ${opened? 'active' : ''}`}>Players</label>
            </div>
            {pressed === 'Players' ? 
